//console.log(Schemas.Task);
//console.log(Schemas.Task.schema());
//console.log(Object.keys(Schemas.Task));
...
/*
Template.expression_filter.events({
    'change': function(event,temp){
        console.log(event);
        console.log(temp);
...
set_filter_selector = function(no){
    ...
    if($('#filter_value' + no ).val()){
            ...
            console.log(Session.get("filter_queries"));
        }
}
...
Template.filter_button.events({
    'click #call_filter': function(){
        console.log(Session.get("filter_queries"));
        console.log(Session.get("filter_selector"));
        ...
        if(Object.keys(Session.get("filter_queries")).length == 1){            
            ...
            console.log(Session.get("filter_ops"));
            var ops = object_values(Session.get("filter_ops"));
            console.log(Session.get("filter_queries"));
            console.log(object_values(Session.get("filter_queries")));
...
Template.filter_button_delete.events({
    'click .delete_filter': function(event){
        ...
        console.log(Session.get("autoform_options"));
        Session.set("autoform_options","{}");
        ...
        if(counter.length == 0){
            Session.set("filter_counter", [1]);
            Blaze.renderWithData(Template.filter_fields, {no: no}, document.getElementById('simple_filter'+'1'));
            Blaze.renderWithData(Template.filter_extra, {no:no}, document.getElementById('simple_filter'+'1'));
            Session.set("filter_selector",'');
        }
        else
            if(Object.keys(Session.get("filter_queries")).length == 1){            
...
Template.filter_operators.events({
    'change .filter_operators': function(event){
        ...
        if(add){
            ...
            console.log('simple_filter' + counter[counter.length-1]);
            $('#field_button').remove();
            Blaze.render(Template.button_locations, document.getElementById('simple_filter' + counter[counter.length-1] ));
        }
    }
